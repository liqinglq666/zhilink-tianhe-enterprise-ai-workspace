# -*- coding: utf-8 -*-
"""Account, organization, RBAC, session, and scoped project access store."""
...
from sqlalchemy import DateTime, ForeignKey, String, Text, UniqueConstraint, delete, exists, func, select
...
ROLE_PERMISSIONS = {
    ...
    "editor": {
        ...
        "history:read", "history:restore",
    },
    "admin": {
        "organization:read", "organization:update", "member:read", "member:manage", "project:read",
        "project:create", "project:update", "project:delete", "project:claim", "history:read", "history:restore",
    },
    "owner": {
        "organization:read", "organization:update", "organization:delete", "member:read", "member:manage",
        "owner:manage", "project:read", "project:create", "project:update", "project:delete", "project:claim",
        "history:read", "history:restore",
    },
}
...
def validate_password(password: str) -> str:
    ...
    if len(value) < minimum:
        raise AccountStoreError(422, "AUTH_PASSWORD_TOO_SHORT", f"密码至少需要 {minimum} 个字符。")
    if len(value) > 128:
...
def verify_password(password: str, encoded: str) -> bool:
    try:
        ...
        actual = hashlib.scrypt((password or "").encode("utf-8"), salt=salt, n=int(n_raw), r=int(r_raw), p=int(p_raw), dklen=len(expected))
...
class AccountStore:
    def __init__(self, projects=None) -> None:
        """Use a database schema that has already been upgraded by Alembic."""
        try:
    ...
    def register(self, *, email: str, password: str, display_name: str, organization_name: str, user_agent: str = "") -> tuple[AuthSession, str, str]:
        ...
        user = UserRecord(id=str(uuid4()), email=normalized, display_name=name, password_hash=hash_password(password))
        organization = OrganizationRecord(id=str(uuid4()), name=org_name, slug=_organization_slug(), created_by_user_id=user.id)
    ...
    def login(self, *, email: str, password: str, user_agent: str = "") -> tuple[AuthSession, str, str]:
        normalized = normalize_email(email)
        try:
            with self.sessions.begin() as session:
                ...
                if user is None or user.status != "active" or not verify_password(password, user.password_hash):
                    raise AccountStoreError(401, "AUTH_INVALID_CREDENTIALS", "邮箱或密码不正确。")
            ...
            raise AccountStoreError(503, "ACCOUNT_STORAGE_UNAVAILABLE", "登录服务暂时不可用，请稍后重试。", retryable=True) from exc

    def _create_session(self, user: UserRecord, *, user_agent: str) -> tuple[AuthSession, str, str]:
        ...
        record = AuthSessionRecord(
            id=str(uuid4()), user_id=user.id, token_hash=_sha256(token), csrf_hash=_sha256(csrf),
            user_agent_hash=_sha256((user_agent or "")[:1000]), expires_at=_now() + timedelta(hours=ttl_hours),
        )
        try:
            with self.sessions.begin() as session:
                session.execute(delete(AuthSessionRecord).where(AuthSessionRecord.expires_at < _now()))
                session.add(record)
                session.flush()
            return AuthSession(record.id, user.id, user.email, user.display_name, record.csrf_hash, record.expires_at), token, csrf
        except SQLAlchemyError as exc:
            raise AccountStoreError(503, "ACCOUNT_STORAGE_UNAVAILABLE", "登录会话创建失败，请稍后重试。", retryable=True) from exc
    ...
    def get_session(self, token: str | None) -> AuthSession | None:
        ...
        try:
            with self.sessions.begin() as session:
                record = session.scalar(select(AuthSessionRecord).where(AuthSessionRecord.token_hash == _sha256(raw)))
                if record is None:
                    return None
                ...
                return AuthSession(record.id, user.id, user.email, user.display_name, record.csrf_hash, record.expires_at)
        except SQLAlchemyError as exc:
            raise AccountStoreError(503, "ACCOUNT_STORAGE_UNAVAILABLE", "登录状态暂时无法读取，请稍后重试。", retryable=True) from exc
    ...
    def verify_csrf(self, auth: AuthSession, header_token: str, cookie_token: str) -> None:
        header, cookie = (header_token or "").strip(), (cookie_token or "").strip()
        if not header or not cookie or not hmac.compare_digest(header, cookie) or not hmac.compare_digest(_sha256(header), auth.csrf_hash):
        ...
        try:
            with self.sessions() as session:
                rows = session.execute(
                    ...
                    .join(OrganizationMembershipRecord, OrganizationMembershipRecord.organization_id == OrganizationRecord.id)
                    .where(OrganizationMembershipRecord.user_id == user_id, OrganizationMembershipRecord.status == "active")
                    .order_by(OrganizationRecord.created_at.asc())
                ).all()
                return [{"id": org.id, "name": org.name, "slug": org.slug, "role": member.role, "created_at": org.created_at} for org, member in rows]
        except SQLAlchemyError as exc:
    ...
    def create_organization(self, user_id: str, name: str) -> dict[str, Any]:
        ...
        org = OrganizationRecord(id=str(uuid4()), name=cleaned, slug=_organization_slug(), created_by_user_id=user_id)
    ...
    def membership(self, user_id: str, organization_id: str) -> dict[str, Any]:
        try:
            with self.sessions() as session:
                ...
                org, member = row
                return {"organization_id": org.id, "organization_name": org.name, "user_id": user_id, "role": member.role}
        except AccountStoreError:
            raise
        except SQLAlchemyError as exc:
        ...
        try:
            with self.sessions() as session:
                ...
                rows = session.execute(
                    ...
                    .where(OrganizationMembershipRecord.organization_id == organization_id, OrganizationMembershipRecord.status == "active")
    ...
    @staticmethod
    def _validate_role_change(actor_role: str, target_role: str, new_role: str | None) -> None:
    ...
    def add_member(self, actor_user_id: str, organization_id: str, email: str, role: Role) -> dict[str, Any]:
        ...
        try:
            with self.sessions.begin() as session:
                ...
                if existing is None:
                    existing = OrganizationMembershipRecord(id=str(uuid4()), organization_id=organization_id, user_id=user.id, role=role)
                    session.add(existing)
                else:
                    existing.status, existing.role, existing.updated_at = "active", role, _now()
                session.flush()
                return {"user_id": user.id, "email": user.email, "display_name": user.display_name, "role": existing.role, "created_at": existing.created_at}
        except AccountStoreError:
            raise
        except SQLAlchemyError as exc:
    ...
    def _owner_count(self, session, organization_id: str) -> int:
        return int(session.scalar(select(func.count(OrganizationMembershipRecord.id)).where(
            OrganizationMembershipRecord.organization_id == organization_id,
            OrganizationMembershipRecord.status == "active",
    ...
    @staticmethod
    def _lock_organization(session, organization_id: str) -> None:
        """Serialize owner mutations so two owners cannot concurrently remove each other."""
        organization = session.scalar(select(OrganizationRecord).where(OrganizationRecord.id == organization_id).with_for_update())
    ...
    def update_member_role(self, actor_user_id: str, organization_id: str, target_user_id: str, role: Role) -> dict[str, Any]:
        try:
            with self.sessions.begin() as session:
                ...
                session.flush()
                return {"user_id": target_user_id, "email": user.email if user else "", "display_name": user.display_name if user else "", "role": role, "created_at": membership.created_at}
        except AccountStoreError:
            raise
        except SQLAlchemyError as exc:
    ...
    def organization_scope(self, user_id: str, organization_id: str, permission: str) -> ProjectScope:
        member = self.require_permission(user_id, organization_id, permission)
        return ProjectScope(kind="organization", storage_hash=_organization_storage_hash(organization_id), organization_id=organization_id, user_id=user_id, role=member["role"])

    def _project_query(self, scope: ProjectScope):
        query = select(ProjectRecord)
        if scope.kind == "organization":
            return query.join(OrganizationProjectRecord, OrganizationProjectRecord.project_id == ProjectRecord.id).where(OrganizationProjectRecord.organization_id == scope.organization_id)
        return query.where(
    ...
    def list_projects(self, scope: ProjectScope, *, limit: int, offset: int, include_archived: bool) -> tuple[list[dict[str, Any]], int]:
        try:
            with self.sessions() as session:
                ...
                records = session.scalars(query.order_by(ProjectRecord.updated_at.desc()).limit(limit).offset(offset)).all()
    ...
    def create_project(self, scope: ProjectScope, *, name: str, description: str, snapshot: dict[str, Any], version_label: str) -> dict[str, Any]:
        ...
        try:
            with self.sessions.begin() as session:
                ...
                session.flush()
                if scope.kind == "organization":
                    session.add(OrganizationProjectRecord(project_id=record.id, organization_id=scope.organization_id or "", created_by_user_id=scope.user_id or ""))
                self.projects._append_history(session, record, change_kind="create", label=version_label, changed_modules=_initial_modules(snapshot))
    ...
    def update_project(self, scope: ProjectScope, project_id: str, *, expected_lock_version: int, name: str | None, description: str | None, status: str | None, snapshot: dict[str, Any] | None, version_label: str) -> dict[str, Any]:
        try:
            with self.sessions.begin() as session:
                ...
                if snapshot is not None: record.snapshot = snapshot
                metadata_changed = record.name != old_name or record.description != old_description or record.status != old_status
                ...
                if not metadata_changed and not snapshot_changed and not version_label:
                    return _project_to_dict(record)
                change_kind = "save"
                if record.status != old_status and not snapshot_changed and record.name == old_name and record.description == old_description:
                ...
                self.projects._append_history(
                    session, record, change_kind=change_kind, label=version_label,
                    changed_modules=_changed_modules(old_snapshot, record.snapshot or {}, metadata_changed=metadata_changed) or ["project"],
                )
            return _project_to_dict(record)
    ...
    def restore_history(self, scope: ProjectScope, project_id: str, version_number: int, *, expected_lock_version: int, version_label: str) -> dict[str, Any]:
        try:
            with self.sessions.begin() as session:
                ...
                source = session.scalar(select(ProjectHistoryRecord).where(ProjectHistoryRecord.project_id == project_id, ProjectHistoryRecord.version_number == version_number))
                if source is None:
                    raise ProjectHistoryNotFound("项目版本不存在。")
                ...
                session.flush()
                self.projects._append_history(
                    session, project, change_kind="restore", label=version_label,
                    changed_modules=_changed_modules(old_snapshot, project.snapshot or {}) or ["project"], source_version_number=version_number,
                )
        ...
        try:
            with self.sessions.begin() as session:
                ...
                for record in records:
                    session.add(OrganizationProjectRecord(project_id=record.id, organization_id=organization_id, created_by_user_id=actor_user_id))
                return len(records)
        except SQLAlchemyError as exc:
