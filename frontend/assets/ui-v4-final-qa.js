...
(() => {
  ...
  function ensureSidebarSemantics() {
    ...
    if (mobile instanceof HTMLButtonElement && sidebar instanceof HTMLElement) {
      ...
      mobile.setAttribute("aria-label", document.body.classList.contains("live-sidebar-open") ? "关闭主导航" : "打开主导航");
    }
  }
  ...
  function ensureLegacyInteractiveSemantics() {
  ...
  function sync() {
    ...
    ensureLegacyInteractiveSemantics();
  ...
  function handleClick(event) {
    if (event.detail === 0 && event.target.closest?.(PAGE_NAV_SELECTOR)) {
      window.setTimeout(() => queueSync({ focusPage: true }), 0);
    }
  }
