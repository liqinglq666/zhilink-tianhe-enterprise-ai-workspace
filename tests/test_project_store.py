...
def make_store(database_url: str) -> ProjectStore:
    store = ProjectStore(database_url)
...
def test_normalize_database_url_supports_sqlite_and_postgres():
    ...
    assert normalize_database_url("postgres://u:p@db/app").startswith("postgresql+psycopg://")
    assert normalize_database_url("postgresql://u:p@db/app").startswith("postgresql+psycopg://")
...
def test_workspace_key_is_hashed_and_never_stored_raw(tmp_path):
    ...
    store = make_store(f"sqlite:///{database}")
...
def test_projects_are_isolated_by_workspace(tmp_path):
    store = make_store(f"sqlite:///{tmp_path / 'projects.db'}")
...
def test_update_uses_optimistic_lock_and_archive_filter(tmp_path):
    ...
    with pytest.raises(ProjectVersionConflict) as conflict:
        store.update(KEY_A, created["id"], expected_lock_version=1, name="过期页面写入")
